 ...
 * @time 18-6-20
 */
public class StepCountPreference {

    private static final String DOWNLOAD_PREF_NAME = "step_couter_prefs";

    private static StepCountPreference m_stInstance;
    ...
    private StepCountPreference(Context context) {
        mContext = context.getApplicationContext();
        mPrefs = mContext.getSharedPreferences(DOWNLOAD_PREF_NAME, Context.MODE_PRIVATE);
        mEditor = mPrefs.edit();
    }

    public static synchronized StepCountPreference getInstance(Context context) {
        if (m_stInstance == null) {
            m_stInstance = new StepCountPreference(context);
        }
        return m_stInstance;
    }
